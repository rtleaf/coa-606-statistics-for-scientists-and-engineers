...
$t = \frac{Value_{observed} - {Value_{expected}}}{{SE_{estimate}}}$.
...
$t_{\alpha=0.05, df = (29)} = -1.699127$

95% CI of $\beta_1$ = $t_{\alpha=0.05, df = 29}SE_{\beta_1}$ }

$\beta_1 = -0.06822828 \pm 0.01719398$

$\beta_1 = -0.0682 \pm 0.0172$

## Prediction and confidence intervals

Sometimes (often) we will want to use the model to predict future values.

For example, suppose we fit a simple linear regression model using hours studied as a predictor variable and exam score as the response variable. 

Using this model, we might predict that a student who studies for 6 hours will receive an exam score of 91.
...
This range of values is known as a 95% *prediction interval* and it’s often more useful to us than just knowing the exact predicted value.
...
$\hat{y} = s_yt_{(\alpha, df = n-2)}(\frac{1}{n}+\frac{(x^*-\bar{x})^2}{(n-1)s^2_x})^{0.5}$
...
$\hat{y} = s_yt_{(\alpha, df = n-2)}(1+\frac{1}{n}+\frac{(x^*-\bar{x})^2}{(n-1)s^2_x})^{0.5}$
