...
plot(x,y, main = "Plot of Simple Linear Regression Model", xlab = "X", ylab = "Y", xlim = c(0,20), ylim = c(0,60), type = 'l', pch=16, xaxt = "n", yaxt = "n",lwd = 2)
...
plot(x, y, xlim = c(0,25), ylim = c(0,45), pch = 1, xlab = "Size of Spider", ylab = "Anxiety")
...
Let's look at a some cases for alternatvie candidate model parameters:
...
+ Two-way (*n* = 2) factors
+ Three-way (*n* = 3) factors
+ Multifactoral anova (*n* >= 3) factors

## Benefit of Factorial Designs

We can look at how variables interact, this process is called 'interaction'.
...
+ Interaction between hangover and lecture topic on sleeping during lectures. In this case the hangover and sleeping are the factors. Levels in each of the two factors could be (hangover levels = True or False) and sleeping levels could be (snoring, nodding off, and drowsy).

An interaction could indicate that a hangover might have more effect on sleepiness during a biology lecture than during a statistics lecture.
      
## An example of factorial design

Testing the effects of the pH of soil and amount of sunlight exposure (full or partial) on plant height:

$y = \beta_0 + \beta_{pH}X_{pH} + \beta_{Sun}X_{Sun} + \beta_{Interaction}X_{pH_i}X_{Sun_i}$.
...
and $\beta_{Interaction} =\beta_{pH\times Sun} = 0$.
